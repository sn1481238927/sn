...
$(function(){
    ...
    layui.use(['form','element'], function() {
        ...
        element.on('tabDelete(xbs_tab)', function(data){
            ...
            /*头部Tab切换左侧效果切换*/
            var currentId = parseInt(tab_list.pop());
            if(currentId > 0){
                if($('.left-nav #nav li').eq(currentId).parent().parent('li').hasClass('open') == false){
                    $('.left-nav #nav li').eq(currentId).parent().parent('li').click();
                }
                $('.left-nav #nav li').eq(currentId).click();
            }else{
                $('.left-nav #nav li').removeClass('tab-current-li');
            }    
        });
    ...
    $('.left-nav #nav').on('click', 'li', function(event) {
        ...
        if($(this).children('.sub-menu').length){
            ...
            /*当前Tab加背景色*/
            if($(this).hasClass('one-open')){
                $(this).addClass('tab-current-li').siblings().find('li').removeClass('tab-current-li');
            }else{
                $(this).addClass('tab-current-li').siblings().removeClass('tab-current-li');
                $("#nav>li:not(.open)").find('ul li').removeClass('tab-current-li');
                $(".one-open").removeClass('tab-current-li');
            }
...
/*
    ...
    h       弹出层高度（缺省调默认值）
*/
function x_admin_show(title,url,w,h,full=false,close=1,maxmin=true,shadeClose=true){
    if (title == null || title == '') {
        title=false;
    };
    if (url == null || url == '') {
...
function x_admin_add_to_tab(title,url,is_refresh,id) {
...
/*页面内部打开导航页*/
function x_admin_open_to_tab(title, url, id) {
    var currentId = id-1;
    if($('.left-nav #nav li').eq(currentId).parent().parent('li').hasClass('open') == false){
        $('.left-nav #nav li').eq(currentId).parent().parent('li').click();
    }
